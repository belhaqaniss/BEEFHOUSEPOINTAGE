...
export const renderPlanningPng=async(db,weekStart,group="cuisine")=>{
  ...
  const lines=[];lines.push(`<rect width="${width}" height="${height}" fill="#fff"/>`,`<text x="${width/2}" y="38" text-anchor="middle" font-size="27" font-weight="800" fill="#a71922">PLANNING ${groupLabel} · SEMAINE DU ${weekStart.split("-").reverse().join("/")} AU ${days[6].split("-").reverse().join("/")}</text>`);
