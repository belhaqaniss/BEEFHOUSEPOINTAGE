...
export const renderPlanningPng=async(db,weekStart,group="cuisine")=>{
  ...
  const isSalle=group==="salle",width=isSalle?3800:1800,nameWidth=isSalle?420:250,columnWidth=(width-nameWidth)/14,rowHeight=isSalle?78:64,headerHeight=isSalle?125:105,height=headerHeight+rowHeight*(employees.length+1)+90;
  ...
  const titleSize=isSalle?38:27,daySize=isSalle?24:16,shiftSize=isSalle?18:12,nameSize=isSalle?22:15,timeSize=isSalle?21:13;
  ...
  const svg=Buffer.from(`<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}"><style>text{font-family:Arial,sans-serif}</style>${lines.join("")}</svg>`);return sharp(svg).png({palette:true,compressionLevel:6}).toBuffer();
};
