...
const sendImage=async(to,png,caption)=>{
  ...
  const form=new FormData();form.set("messaging_product","whatsapp");form.set("type","image/png");form.set("file",new Blob([png],{type:"image/png"}),"planning-cuisine.png");
...
const findEmployee=(db,text)=>{
  const kitchen=db.prepare("SELECT id,first_name AS first,last_name AS last,role FROM employees WHERE active=1 AND lower(role) LIKE '%cuisine%' ORDER BY length(first_name||last_name) DESC").all();
  const normalized=normalize(text),aliasMap=aliases();
  for(const [alias,target] of aliasMap)if(new RegExp(`(^| )${alias}( |$)`).test(normalized)){const found=kitchen.find(employee=>normalize(`${employee.first} ${employee.last}`)===target);if(found)return found}
  const exact=kitchen.find(employee=>normalized.includes(normalize(`${employee.first} ${employee.last}`)));if(exact)return exact;
  const partial=kitchen.filter(employee=>normalized.split(" ").includes(normalize(employee.first))||normalized.split(" ").includes(normalize(employee.last)));
...
export const parseWhatsAppCommand=(db,input)=>{
  ...
  if(text.includes("planning")&&text.includes("cuisine"))return {type:"planning",weekStart:iso(addDays(mondayOf(parisToday()),text.includes("semaine prochaine")?7:0))};
  ...
  const employee=findEmployee(db,input);if(!employee)return {type:"error",message:"Employé Cuisine introuvable. Écrivez son prénom et son nom complets."};
  ...
  if(operation==="delete")return {type:"pending",action:"delete",employeeId:employee.id,employeeName:`${employee.first} ${employee.last}`,workDate,service};
  ...
  return {type:"pending",action:"set",employeeId:employee.id,employeeName:`${employee.first} ${employee.last}`,workDate,service,startMinutes,endMinutes,closing};
};
...
const scheduleRows=(db,weekStart)=>{
  const weekEnd=iso(addDays(new Date(`${weekStart}T12:00:00Z`),6));
  const employees=db.prepare("SELECT id,first_name AS first,last_name AS last FROM employees WHERE active=1 AND lower(role) LIKE '%cuisine%' ORDER BY first_name,last_name").all();
...
export const renderKitchenPlanningPng=async(db,weekStart)=>{
  const {employees,blocks,closings}=scheduleRows(db,weekStart),days=Array.from({length:7},(_,index)=>iso(addDays(new Date(`${weekStart}T12:00:00Z`),index)));
  const width=1800,nameWidth=250,columnWidth=(width-nameWidth)/14,rowHeight=64,headerHeight=105,height=headerHeight+rowHeight*(employees.length+1)+90;
  ...
  const lines=[];lines.push(`<rect width="${width}" height="${height}" fill="#fff"/>`,`<text x="${width/2}" y="38" text-anchor="middle" font-size="27" font-weight="800" fill="#a71922">PLANNING CUISINE · SEMAINE DU ${weekStart.split("-").reverse().join("/")} AU ${days[6].split("-").reverse().join("/")}</text>`);
  lines.push(`<rect x="0" y="55" width="${width}" height="50" fill="#333"/>`,`<text x="12" y="86" font-size="17" font-weight="700" fill="#fff">EMPLOYÉ</text>`);
  days.forEach((date,index)=>{const x=nameWidth+index*columnWidth*2;lines.push(`<text x="${x+columnWidth}" y="78" text-anchor="middle" font-size="16" font-weight="700" fill="#fff">${dayNames[index]}</text>`,`<text x="${x+columnWidth/2}" y="99" text-anchor="middle" font-size="12" fill="#eee">Matin</text>`,`<text x="${x+columnWidth*1.5}" y="99" text-anchor="middle" font-size="12" fill="#eee">Soir</text>`)});
  employees.forEach((employee,row)=>{const y=headerHeight+row*rowHeight,fill=row%2?"#f7f7f7":"#fff";lines.push(`<rect x="0" y="${y}" width="${width}" height="${rowHeight}" fill="${fill}"/>`,`<text x="12" y="${y+38}" font-size="15" font-weight="700" fill="#222">${escapeXml(`${employee.first} ${employee.last}`)}</text>`);days.forEach((date,day)=>["matin","soir"].forEach((service,shift)=>{const x=nameWidth+(day*2+shift)*columnWidth,value=cell(employee.id,date,service);if(value)lines.push(`<rect x="${x+2}" y="${y+5}" width="${columnWidth-4}" height="${rowHeight-10}" rx="5" fill="${service==="matin"?"#f1d56e":"#d7e6f7"}"/>`,`<text x="${x+columnWidth/2}" y="${y+37}" text-anchor="middle" font-size="13" font-weight="700" fill="#222">${escapeXml(value)}</text>`)}))});
...
const handleMessage=async(db,phone,text)=>{
  ...
  if(command.type==="help"){await sendText(phone,commandHelp());return}
  ...
  if(command.type==="confirm"){
    ...
    const action=JSON.parse(pending.payload);applyAction(db,phone,action);await sendText(phone,`✅ Planning Cuisine mis à jour.\n\n${description(action)}`);return;
  }
  if(command.type==="planning"){const png=await renderKitchenPlanningPng(db,command.weekStart);await sendImage(phone,png,`Planning Cuisine · semaine du ${command.weekStart.split("-").reverse().join("/")}`);return}
  ...
  await sendText(phone,`⚠️ Confirmez cette modification :\n\n${description(command)}\n\nRépondez CONFIRMER dans les 10 minutes, ou ANNULER.`);
};
...
export const createWhatsAppHandler=({db,json})=>async(req,res,url)=>{
  ...
  for(const message of messages){if(message.type!=="text")continue;const phone=digits(message.from);handleMessage(db,phone,message.text?.body||"").catch(error=>{console.error("Erreur bot WhatsApp:",error);sendText(phone,"Une erreur est survenue. Réessayez ou contactez l’administrateur.").catch(console.error)})}
};
