...
type Block={employeeId:number;workDate:string;startMinutes:number;service?:"matin"|"soir"};
...
export default function Planning({employees,request}:{employees:Employee[];request:(payload:object)=>Promise<any>}){
  ...
  const[manual,setManual]=useState(()=>({employeeId:"",workDate:iso(new Date()),service:"matin" as "matin"|"soir",start:"09:00",end:"17:00"}));
  ...
  return <article className="super-panel planning-panel">
    ...
    <form className="manual-planning" onSubmit={saveManual}><div><strong>Méthode 2 · saisie manuelle</strong><small>Ajoutez séparément le service du matin et celui du soir. Les cubes seront mis à jour automatiquement.</small></div><label>Employé<select required value={manual.employeeId} onChange={e=>setManual({...manual,employeeId:e.target.value})}>{activeEmployees.map(employee=><option value={employee.id} key={employee.id}>{employee.first} {employee.last}</option>)}</select></label><label>Date<input required type="date" min={iso(new Date())} value={manual.workDate} onChange={e=>setManual({...manual,workDate:e.target.value})}/></label><label>Service<select value={manual.service} onChange={e=>{const service=e.target.value as "matin"|"soir";setManual({...manual,service,start:service==="matin"?"09:00":"18:00",end:service==="matin"?"17:00":"02:00"})}}><option value="matin">Matin</option><option value="soir">Soir</option></select></label><label>Début<input required type="time" step="1800" value={manual.start} onChange={e=>setManual({...manual,start:e.target.value})}/></label><label>Fin<input required type="time" step="1800" value={manual.end} onChange={e=>setManual({...manual,end:e.target.value})}/></label><button disabled={manualBusy||!manual.employeeId}>{manualBusy?"Enregistrement…":`Appliquer ${manual.service}`}</button>{manualError&&<p className="super-alert">{manualError}</p>}</form>
    <div className="planning-help"><i/> Le matin et le soir sont indépendants : modifier l’un ne supprime plus l’autre. Ouverture 07:00, fermeture 02:00.</div>
