...
export const employeeSummary=(person:ExcelPerson,records:ExcelAttendance[],date:string)=>{
  ...
  for(const event of events){if(event.type==="Arrivée"){const arrival=new Date(event.timestamp),arrivalTime=parisTime(arrival),period=arrivalTime.hour>=13?"soir":"matin",scheduled=period==="soir"?event.scheduledEveningStartMinutes:event.scheduledMorningStartMinutes,earlyMinutes=scheduled==null?0:scheduled-arrivalTime.minutes;let start=arrival;if(earlyMinutes>0&&earlyMinutes<=15)start=new Date(arrival.getTime()+earlyMinutes*60000);shifts.push({start,period})}else{const open=[...shifts].reverse().find(shift=>shift.start&&!shift.end);if(open)open.end=new Date(event.timestamp)}}
