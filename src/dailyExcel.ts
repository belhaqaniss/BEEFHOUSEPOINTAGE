...
export const employeeSummary=(person:ExcelPerson,records:ExcelAttendance[],date:string)=>{
  ...
  for(const event of events){if(event.type==="Arrivée"){const arrival=new Date(event.timestamp),arrivalTime=parisTime(arrival),period=arrivalTime.hour>=15?"soir":"matin",scheduled=period==="soir"?event.scheduledEveningStartMinutes:event.scheduledMorningStartMinutes;let start=arrival;if(scheduled!=null&&arrivalTime.minutes<scheduled)start=new Date(arrival.getTime()+(scheduled-arrivalTime.minutes)*60000);shifts.push({start,period})}else{const open=[...shifts].reverse().find(shift=>shift.start&&!shift.end);if(open)open.end=new Date(event.timestamp)}}
