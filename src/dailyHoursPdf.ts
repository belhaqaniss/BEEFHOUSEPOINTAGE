...
export function buildDailyHoursPdf(date:string,people:ExcelPerson[],records:ExcelAttendance[]){
  ...
  const rowCount=people.length+groups.length,rowHeight=Math.max(13,Math.min(19,410/Math.max(rowCount,1)));
  ...
  for(const group of groups){
    stream+=fill(x,y,tableWidth,rowHeight,group.color)+border(x,y,tableWidth,rowHeight)+text(x+7,y+rowHeight/2-3,8,group.label,true);
    y-=rowHeight;
    for(const person of group.people){
      ...
      cursor=x;
      values.forEach((value,index)=>{const bg=index===3?"0.99 0.90 0.80":index===6||index===7?"1 0.95 0.80":"1 1 1";stream+=fill(cursor,y,widths[index],rowHeight,bg)+border(cursor,y,widths[index],rowHeight)+text(cursor+5,y+rowHeight/2-3,index===0?8:7.5,value,index===7);cursor+=widths[index]});
      y-=rowHeight;
    }
  }
  stream+=text(34,18,7,"Document genere depuis BEEF HOUSE")+text(680,18,7,new Date().toLocaleString("fr-FR"));
